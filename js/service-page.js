(function () {
    ...
    <section class="page-hero">
      <div class="container page-hero__inner">
        ...
        <div class="page-hero__image reveal">
          <img src="../${service.image}" alt="${service.title}" width="480" height="480">
        </div>
      </div>
    </section>
  ...
  if (slug === 'sale' && KWT.saleCatalog) {
    const grid = document.getElementById('sale-grid');
    if (grid) {
      grid.innerHTML = KWT.saleCatalog.map(item => KWTProducts.renderCard(item, 'продажа')).join('');
